...
import { StreamPanel } from "../../../../../components/graph/StreamPanel";
...
export default function WorkspaceGraphsPage() {
  ...
  const streamEnabled = Boolean(activeGraphId);
  const { events, status: streamStatus, error: streamError, reset: resetStream } =
    useGraphStream(activeGraphId, streamEnabled);

  const loadData = useCallback(async () => {
    if (!workspaceId) return;
    const [docs, graphRows] = await Promise.all([
      listDocuments(workspaceId),
      listGraphs(workspaceId),
    ]);
    setDocuments(docs);
    setGraphs(graphRows);
  }, [workspaceId]);
  ...
  useEffect(() => {
    ...
    loadData()
      .catch((loadError) => {
        if (mounted) setError(loadError instanceof Error ? loadError.message : "Failed to load");
      })
      .finally(() => { if (mounted) setLoading(false); });
    return () => { mounted = false; };
  }, [loadData]);

  // Reload graph list when stream signals completion
  useEffect(() => {
    if (streamStatus === "complete") {
      loadData().catch(() => undefined);
    }
  }, [streamStatus, loadData]);
  ...
  async function triggerBuild() {
    if (!workspaceId || selectedDocumentIds.length < 2) return;
    setSubmitting(true);
    setError(null);
    resetStream();
    try {
      const response = await buildGraph(workspaceId, selectedDocumentIds);
      setActiveGraphId(response.graph.id);
    } catch (buildError) {
  ...
  const isBuilding = streamEnabled && streamStatus !== "complete" && streamStatus !== "error";
      ...
      <div className="mb-6 glass rounded-2xl p-5">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <button
            ...
            disabled={selectedDocumentIds.length < 2 || submitting || isBuilding}
          ...
          >
            {submitting || isBuilding ? (
              <>
                <svg className="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                  <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" strokeLinecap="round"/>
                </svg>
                {isBuilding ? "Analyzing…" : "Starting…"}
              </>
            ) : (
              <>
          ...
          {selectedDocumentIds.length < 2 && !isBuilding && (
            <p className="text-xs text-ink-3">Select at least 2 documents</p>
          )}
        </div>

        {error && (
          <div className="mt-4 rounded-xl border border-danger/25 bg-danger/8 px-4 py-3 text-sm text-red-300">
            {error}
          </div>
        )}

        {/* SSE stream panel — shown while building or after completion */}
        {activeGraphId && (
          <StreamPanel events={events} status={streamStatus} error={streamError} />
        )}
      </div>
